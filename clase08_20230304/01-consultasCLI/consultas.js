...
// load("C:\\Users\\diego\\dev\\CoderHouse\\DesafiosBackend-main\\Clase10Websockets\\ejemplos\\pruebas\\02-mongod\\consultas.js");
...
// ******************
// READ: consultas / busquedad en una colección, con find(opt) -opciones / FILTROS 
// ******************

db = db.getSiblingDB('pruebas_mongo');

db.mascotas.find({ edad: { $lt: 5 } },{_id:0, nombre:1, edad:1});
db.mascotas.find({ edad: { $lte: 5 } });
db.mascotas.find({ edad: { $gt: 5 } });
db.mascotas.find({ edad: { $gte: 7 } });
...
// debe ejecutarse para una propiedad que sea un array de objetos. Caso contrario no tendría
// elementos para hacer match... en la siguiente consulta no tendría sentido utilizar
// el operador $elemMatch:
db.mascotas.find(
    { especie: { $elemMatch: { especie:'gato' } } }
);
...
db.mascotas.find({ 'enemigos.domicilio.calle': { $in: ['las Bases', 'Maipu'] } },{_id:0, 'enemigos.nombre':1, 'enemigos.domicilio.calle':1});
...
db.mascotas.find(
    {
        'enemigos.domicilio.calle': /las/ig 
    }, 
    { 
        _id: 0, nombre: 1, edad: 1, 'enemigos.domicilio.calle':1 
    });
...
// skip
db.mascotas.find({}, { nombre: 1 }).sort({ especie:1, nombre:1 }).skip(5);
...
db.mascotas.updateOne({ 'nombre': 'Correcaminos' }, { $set: { 'edad': 12 } })
...
// Notar que tengo que enviar el objeto de filtro si o si. Pero lo envío vacío, lo que
// indica que no hay filtros. Por lo tanto el updateMany se realizará dobre todos
// los documentos de mi colección
...
// toArray():
let mascotas=db.mascotas.find().toArray();
mascotas.forEach(mascota=>{
    if(mascota.peliculas){
        mascota.peliculas.forEach(peli=>{
            if(peli.espectadores>=500000){
                console.log(mascota.nombre, peli.titulo, peli.espectadores)
            }
        })
    }
})

// En mongosh puedo ejecutar JS. Un .find() devuelve una estructura llamada cursor, 
// que no es exactamente un array. Pueden investigar cursores, para ver qué son y 
// como se trabaja con estos. Con .toArray, pueden transformar el cursor devuelto 
// por un .find(). Transformarlo en un array. Luego pueden operar desde mongosh con 
// el array, con JS. Haciendo un forEach, o un map, lo que necesiten. 

let heroes=db.heroes.find().toArray();
heroes.map(hero => { return 'Me llamo ' + hero.nombre });
